...
export function buildPrompt(topic, difficulty, todayHistory) {
...
Return ONLY valid JSON (no markdown, no preamble):
{
  ...
  "vocab": [
    { "de": "word", "en": "translation", "note": "gender/usage or null", "kita": true }
  ],
  "sentences": [
...
- sentences: exactly 3, at least 1 usable with Walter
...
- fill_in exercises use: { "type":"fill_in", "prompt":"...", "question":"sentence with ___", "correct_answer":"exact word(s)", "explanation":"why correct + what a wrong answer looks like and why it fails" }
...
export function buildReviewPrompt(difficulty) {
...
This is a review — no new grammar. Cross-test ALL priority gaps across the 10 exercises.
...
Return ONLY valid JSON (no markdown, no preamble):
{
  ...
  "sentences": [
    { "de": "sentence with deliberate error", "en": "what it should mean (spot the error)" },
    { "de": "correct seit + Praesens with Walter", "en": "translation" },
    { "de": "correct Konjunktiv II daily-life sentence", "en": "translation" }
  ],
  "exercises": [
    {
      "type": "multiple_choice",
...
Rules:
- vocab: exactly 5 items, 1-2 kita:true
- sentences: exactly 3; sentences[0] must contain a deliberate error
- exercises: exactly 10, mix of multiple_choice and fill_in only, each targeting a different gap from the priority list
- fill_in uses: { "type":"fill_in", "prompt":"...", "question":"...", "correct_answer":"...", "explanation":"..." }
